...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = selectedTopic
      ? `Generate a math word problem suitable for a Primary 5 student on the topic: ${selectedTopic}. Return only valid JSON with no extra text or formatting: {"problem_text": "[word problem]", "final_answer": [numerical answer]}`
      : "Generate a math word problem suitable for a Primary 5 student. Return only valid JSON with no extra text or formatting: {\"problem_text\": \"[word problem]\", \"final_answer\": [numerical answer]}";
    ...
    const { data: session, error } = await supabase
      ...
      .insert({ problem_text: data.problem_text, correct_answer: data.final_answer })
      .select('id')
      .single();
    ...
    return NextResponse.json({ sessionId: session.id, problem_text: data.problem_text, topic: selectedTopic });
  } catch (error) {
